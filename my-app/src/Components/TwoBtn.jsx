...
const TwoBtn = ({
  orderLabel = "Get A Quote",
  whatsappLabel = "Chat With us!",
  orderBgColor = "bg-blue",
  orderTextColor = "text-white",
  whatsappBgColor = "bg-transparent",
  whatsappTextColor = "text-blue",
  whatsappBorderColor = "border-blue",
}) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const openModal = () => setIsModalOpen(true);
  const closeModal = () => setIsModalOpen(false);
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row gap-2 py-2 w-full md:w-auto">
        {/* Order Button */}
        <div
          className={`group ${orderBgColor} w-full md:w-[160px] h-10 flex gap-2 items-center justify-center hover:bg-secondary cursor-pointer transition duration-300 ease-in-out rounded-[25px]`}
        >
          <button onClick={openModal} className={`${orderTextColor} text-sm`}>
            {orderLabel}
          </button>
        </div>
        ...
        >
          <div
            className={`group border ${whatsappBorderColor} ${whatsappBgColor} w-full md:w-[160px] h-10 flex gap-2 items-center justify-center rounded-[25px] hover:bg-blue transition duration-300 ease-in-out`}
          >
            <h1
              className={`${whatsappTextColor} text-sm group-hover:text-white transition duration-300 ease-in-out`}
            >
              {whatsappLabel}
            </h1>
          </div>
