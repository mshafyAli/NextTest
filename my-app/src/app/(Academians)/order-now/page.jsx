...
const OrderNowPage = () => {
  ...
  // Set page title dynamically using Next.js's Head component
  useEffect(() => {
    document.title = 'Order Now - Research House Publication.'; 
  }, []);
  ...
  useEffect(() => {
    const fetchLastOrderNumber = async () => {
      try {
        const response = await axios.get(`https://be.researchpublishinghouse.com/api/last-order-number.php`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://be.researchpublishinghouse.com/api/order-now.php`, orderData);

      if (response.status === 201 || response.status === 200) {
        const redirectUrl = `https://webmountx.com/payments/RPH/?&NAME_KEY=${encodeURIComponent(name)}&EMAIL_KEY=${encodeURIComponent(email)}&TOTAL_AMOUNT=${encodeURIComponent(totalAmount)}&ORDER_NUMBER=${encodeURIComponent(orderNumber)}`;
  ...
  return (
    <>
      ...
      <Head>
        <title>Order Now - Research House Publication.</title>
      </Head>
      ...
      <div className="bg-white">
        ...
        <p className="text-center text-black mb-6 w-full max-w-md mx-auto">
          Place your order now at Research House Publications and get the premium quality 100%
        ...
        <div className="flex justify-center items-center">
          <div className="bg-gray-800 p-8 rounded-lg shadow-md w-full max-w-md border-4 border-orange-400">
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {/* Submit Button */}
              <button
                className="w-full py-3 bg-blue-900 text-white font-bold bg-blue rounded-md hover:bg-blue focus:outline-none focus:bg-blue"
                type="submit"
                disabled={loading}
              >
